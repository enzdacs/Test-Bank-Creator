...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // API Endpoint for Gemini 1.5 Flash (stable version)
    const url = `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`;
    ...
    if (data.candidates && data.candidates[0] && data.candidates[0].content) {
      ...
      // Itatawag ang error na ito kung na-block ng safety filters o walang na-generate
      console.error("Gemini Empty Response:", JSON.stringify(data));
      return {
        statusCode: 500,
        body: JSON.stringify({ 
          error: "No content in API response. This might be due to safety filters or an invalid prompt.",
          details: data 
        })
      };
    }
